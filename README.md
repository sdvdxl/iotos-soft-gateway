...
### 下发命令处理
框架封装了下发命令处理过程，只需要实现 `SubsystemCommandService` 接口并添加注解 `CloudSendCommand`。

cmd，type， gateway 是 与 的关系。
...
队列只是暂时缓冲，频率高的话，最终队列也会满，队列设置最好不要超过10000.
